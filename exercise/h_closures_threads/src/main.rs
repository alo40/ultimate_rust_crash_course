...
fn main() {
    ...
    //
    let handle = thread::spawn(move || expensive_sum(my_vector));  // a block {} is an expression, but a function is also an expression
    ...
    tx1.send("send message 1.1 to thread").unwrap();
    tx2.send("send message 2.1 to thread").unwrap();

    tx1.send("send message 1.2 to thread").unwrap();
    tx2.send("send message 2.2 to thread").unwrap();
    ...
    // join threads for hygienic reasons
